...
public class RainMotionElement extends DisplayElement{
    ...
    Frame s;
    RainMotionElement(String id, ClockDisplaySystem clockDisplaySystem, int size, int row, int col, long updateInterval, Engine e)
    {
    ...
    JsonObject[] get(long time) {
        ...
        if(startLocation + imageHeight <= s.getHeight()) {
            ...
            current.addProperty("n", s.frameNumber);
